...
for (root,dirs,files) in os.walk(path):
    if len(files) > 0:
        ...
        for j in files:
            ...
            print(z)
            o = []
            with open(root + '/' + j, 'rb') as f1:
                d = pickle.load(f1)
            d[0] = cv.cvtColor(d[0],cv.COLOR_BGR2GRAY)
            for i in range(1,len(d)):
                # o.append(opticalflow(d[i-1],d[i]))
                d[i] = cv.cvtColor(d[i],cv.COLOR_BGR2GRAY)
                # d[i-1] = cv.cvtColor(d[i-1],cv.COLOR_BGR2GRAY)
                d[i-1] = cv.medianBlur(d[i-1],5)
                d[i] = cv.medianBlur(d[i],5)
                # print(d[i].shape, d[i-1].shape)
                flow = cv.calcOpticalFlowFarneback(d[i-1],d[i], None, 0.5,3,7,4,7,5, 0)
                o.append(flow)
            with open('./STAIR_Lab_optical/' + root[len(path):] + '/' + j, 'wb') as f1:
                pickle.dump(o,f1)
                print(j)
            # np.save('./STAIR_Lab_optical/' + root[len(path):] + '/' + j,o)